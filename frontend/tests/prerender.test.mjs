...
//
//   1. The static HTML comes out empty. This is not hypothetical — it is how
//      the work started. AuthProvider rendered `{!loading && children}`, so
//      the entire application waited on a session check that never runs in
//      Node, and every page prerendered to zero bytes.
...
// /how-it-works with the SPA shell, while Netlify answers it with
...
// fallback, which is index.html — the home page's markup, in the root. They
// are here because that is exactly the case that breaks: main.jsx would see a
// non-empty root and hydrate, reconciling a results screen against a landing
// page, and every one of these pages would log React #418 in production while
// quietly throwing the markup away. A prerendered-only test cannot see it.
const FALLBACK_ROUTES = ['/results/does-not-exist', '/elections'];
...
for (const route of [...ROUTES, ...FALLBACK_ROUTES]) {
  ...
  if (!title.includes('EVSPolls')) failures.push(`${route}: unexpected title ${JSON.stringify(title)}`);
